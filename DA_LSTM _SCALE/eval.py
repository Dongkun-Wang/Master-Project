...
def evaluation(saved_model_path, mode='test'):
    batch_size = 1
    a = CreateStockSamples(hp.input_time_step, hp.output_time_step)
    x_test, y_test, scaler = a.create_samples(mode)
    scale = scaler.data_range_
    feature = x_test.shape[-1]  # value:81
    # build the model ##################################################################################################
    da = DaRnn(hp.input_time_step, hp.output_time_step, feature, hp.num_layers, hp.drop_out_rate, batch_size)
    ...
    with tf.Session(config=config) as sess:
        ...
        for j in range(0, x_test.shape[0], batch_size):
            end_batch = np.minimum(x_test.shape[0], j + batch_size)
        ...
        test_RMSE = np.mean(test_RMSE * scale)
        test_MAE = np.mean(test_MAE * scale)
        test_MAPE = np.mean(test_MAPE)
        # print test and best result ###############################################################################
        print('test RMSE: {:.5f} | test MAE {:.5f} | test MAPE {:.5f}'.format(test_RMSE, test_MAE, test_MAPE))
        plt.plot(output, label=mode + ' predict')
        plt.plot(y_test, label='truth value')
        plt.legend()
        plt.show()
...
if __name__ == '__main__':
    test_RMSE, test_MAE, test_MAPE = evaluation('saved_model/best_model', mode='valid')
